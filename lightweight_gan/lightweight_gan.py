...
class Trainer():
    def __init__(
        ...
        **kwargs
        ...
        self.GAN = None
        self.device = device  # Device will be set by Accelerator
        self.device_type = get_device_type(device) if device else 'cpu'
    ...
    def set_data_src(self, folder):
        ...
        sampler = None  # Let Accelerator handle the sampler
        dataloader = DataLoader(self.dataset, num_workers = num_workers, batch_size = math.ceil(self.batch_size / self.world_size), sampler = sampler, shuffle = True, drop_last = True, pin_memory = True)
    ...
    def train(self):
        ...
        for i in gradient_accumulate_contexts(self.gradient_accumulate_every, self.is_ddp, ddps=[D]):
        ...
        for i in gradient_accumulate_contexts(self.gradient_accumulate_every, self.is_ddp, ddps=[G]):
            latents = torch.randn(batch_size, latent_dim).to(self.device)
